...
public partial class ListSuppliers : System.Web.UI.UserControl
{
    SystemUser user;
    MSystemService client = new MSystemService();
    Bussinesslogic bll = new Bussinesslogic();
    public event EventHandler RegisterPurchaseEvent;
    ...
    protected void dataGridResults_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        ...
        if (e.CommandName.Equals("EditEntity"))
        {
            ...
            //Server.Transfer("~/AddOrEditSupplier.aspx?Id=" + Id + "&BankCode=" + BankCode);
            return;
        }
        if (e.CommandName.Equals("RegisterPurchase"))
        {
            if (RegisterPurchaseEvent != null)
            {
                MyEventArgs eventArgs = new MyEventArgs();
                eventArgs.PegPayId = Id;
                eventArgs.RequestID = "";

                RegisterPurchaseEvent(sender, eventArgs);
            }
        }
    }
    ...
    private void SearchDb()
    {
        ...
        DataTable dt = bll.SearchSuppliersTable(searchParams);
        if (dt.Rows.Count > 0)
        {
